   ...
   async.waterfall([
       function (cb) {
          cnn.chkQry('select * from User where email = ?',
           [req.params.username], cb);
       },
       function (rvw, fields, cb) {
          if (vld.check(rvw.length, Tags.notFound, null, cb) &&
           vld.checkPrsOK(rvw[0].id, cb))
             cnn.chkQry('delete from Sentiment where sntId = ?',
              [req.params.sntId], cb);
       }],
    function (err) {
       if (!err)
